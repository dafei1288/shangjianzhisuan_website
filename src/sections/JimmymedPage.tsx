...
export default function JimmymedPage() {
  const navigate = useNavigate();
  const { siteConfig, jimmymedConfig, pageLabels } = useConfigs();
  ...
  return (
    <div style={{ background: '#060D1A', minHeight: '100vh', position: 'relative', overflowX: 'hidden' }}>
      ...
      <main style={{ position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <Parallax speed={0.12}>
              <div className="flex flex-wrap items-center" style={{ gap: 16, marginTop: 56 }}>
                <a
                  href={jimmymedConfig.huggingfaceUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="nav-link"
                  style={{
                ...
                <a
                  href={jimmymedConfig.modelscopeUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="nav-link"
                  style={{
        ...
        <section style={{ padding: '0 5vw 150px', position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 1, background: 'rgba(0, 180, 216, 0.1)' }}>
              {jimmymedConfig.features.map((f, i) => (
                <div
                  key={f.title}
            ...
            <div ref={(el) => { revealRefs.current[1] = el; }}>
              <div className="flex flex-col md:flex-row" style={{ gap: 60 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.jimmymed[0]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{jimmymedConfig.quickstart.install}</pre>
                </div>
              </div>
              <div className="flex flex-col md:flex-row" style={{ gap: 60, marginTop: 80 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.jimmymed[1]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{jimmymedConfig.quickstart.python}</pre>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Medical QA — mirrors CinematicVision title/desc split */}
        <section style={{ padding: '0 5vw 150px', background: '#060D1A', position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            <SectionHead label={pageLabels.jimmymed.qa} />
            <div ref={(el) => { revealRefs.current[2] = el; }}>
              <div className="flex flex-col md:flex-row md:items-start" style={{ gap: 60, marginBottom: 48 }}>
                ...
                <Parallax speed={0.07} style={{ flex: '1 1 50%' }}>
                  ...
                  >
                    {pageLabels.jimmymed.qaDesc}
                  </p>
                </Parallax>
              </div>
              <div className="grid grid-cols-1" style={{ gap: 16 }}>
                {jimmymedConfig.examples.map((q) => (
                  <div
                    key={q}
                    style={{
                      background: '#0B1626',
                  ...
                  >
                    <span style={{ color: '#FF8C42' }}>Human: </span>
                    {q}
                  </div>
                ))}
              </div>
        ...
        {/* Disclaimer */}
        <section style={{ padding: '0 5vw 120px', background: '#060D1A', position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            <div
            ...
            >
              ...
              >
                {pageLabels.jimmymed.disclaimerTitle}
              </div>
              <p
                style={{
                  fontFamily: "'Inter', sans-serif",
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 5vw 120px', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
            ...
            <a
              ...
              style={{ color: '#FF8C42' }}
            >
              {pageLabels.common.viewOnHuggingFace}
            </a>
          </div>
        </div>
