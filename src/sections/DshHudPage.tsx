...
export default function DshHudPage() {
  const navigate = useNavigate();
  const { siteConfig, dshHudConfig, pageLabels } = useConfigs();
            ...
            <div ref={(el) => { revealRefs.current[1] = el; }}>
              <div className="flex flex-col md:flex-row md:items-start" style={{ gap: 60, marginBottom: 48 }}>
                <Parallax speed={0.16} style={{ flex: '0 0 50%' }}>
                  ...
                  >
                    {pageLabels.hud.dshTitle}
                  </h2>
                </Parallax>
                <Parallax speed={0.07} style={{ flex: '1 1 50%' }}>
                  <div className="flex flex-col" style={{ gap: 10 }}>
                    {dshHudConfig.previewNotes.map((n) => (
                      <p
                        key={n.label}
                        style={{
                          fontFamily: "'Inter', sans-serif",
            ...
            <div ref={(el) => { revealRefs.current[2] = el; }}>
              <div className="flex flex-col md:flex-row" style={{ gap: 60 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.dshHud[0]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{dshHudConfig.quickstart.build}</pre>
                </div>
              </div>
              <div className="flex flex-col md:flex-row" style={{ gap: 60, marginTop: 80 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.dshHud[1]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{dshHudConfig.quickstart.install}</pre>
                </div>
              </div>
              <div className="flex flex-col md:flex-row" style={{ gap: 60, marginTop: 80 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.dshHud[2]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{dshHudConfig.quickstart.pricing}</pre>
                </div>
              </div>
            </div>
          </div>
