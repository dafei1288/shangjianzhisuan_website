...
export default function PiAgentHudPage() {
  const navigate = useNavigate();
  const { siteConfig, piAgentHudConfig, pageLabels } = useConfigs();
  ...
  return (
    <div style={{ background: '#060D1A', minHeight: '100vh', position: 'relative', overflowX: 'hidden' }}>
      ...
      <main style={{ position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <Parallax speed={0.12}>
              <div className="flex flex-wrap items-center" style={{ gap: 16, marginTop: 56 }}>
                <a
                  href={piAgentHudConfig.githubUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="nav-link"
                  style={{
        ...
        <section style={{ padding: '0 5vw 150px', position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 1, background: 'rgba(0, 180, 216, 0.1)' }}>
              {piAgentHudConfig.features.map((f, i) => (
                <div
                  key={f.title}
        ...
        {/* HUD preview — mirrors CinematicVision title/desc split */}
        <section style={{ padding: '0 5vw 150px', background: '#060D1A', position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            <SectionHead label={pageLabels.hud.previewSection} />
            <div ref={(el) => { revealRefs.current[1] = el; }}>
              <div className="flex flex-col md:flex-row md:items-start" style={{ gap: 60, marginBottom: 48 }}>
                <Parallax speed={0.16} style={{ flex: '0 0 50%' }}>
                  ...
                  >
                    {pageLabels.hud.piTitle}
                  </h2>
                </Parallax>
                <Parallax speed={0.07} style={{ flex: '1 1 50%' }}>
                  <div className="flex flex-col" style={{ gap: 10 }}>
                    {piAgentHudConfig.previewNotes.map((n) => (
                      <p
                        key={n.label}
                        style={{
                          fontFamily: "'Inter', sans-serif",
            ...
            <div ref={(el) => { revealRefs.current[2] = el; }}>
              <div className="flex flex-col md:flex-row" style={{ gap: 60 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.piAgentHud[0]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{piAgentHudConfig.quickstart.install}</pre>
                </div>
              </div>
              <div className="flex flex-col md:flex-row" style={{ gap: 60, marginTop: 80 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.piAgentHud[1]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{piAgentHudConfig.quickstart.config}</pre>
                </div>
              </div>
              <div className="flex flex-col md:flex-row" style={{ gap: 60, marginTop: 80 }}>
                <Parallax speed={0.12} style={{ flex: '0 0 30%' }}>
                  <h3 style={stepTitleStyle}>{pageLabels.steps.piAgentHud[2]}</h3>
                </Parallax>
                <div style={{ flex: '1 1 70%' }}>
                  <pre style={codeStyle}>{piAgentHudConfig.quickstart.plugin}</pre>
                </div>
              </div>
            </div>
          </div>
